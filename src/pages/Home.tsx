  ...
  return (
    <article className={`group ${featured ? '' : 'border-b border-gray-100 pb-5 last:border-0'}`}>
      <Link to={`/${article.primarySection?.toLowerCase()}/${article.slug}`}>
        ...
        <h3 className={`font-display font-semibold text-green-dark group-hover:text-green transition-colors ${featured ? 'text-xl lg:text-2xl' : 'text-base'}`}>
          {article.title}
        </h3>
...
export function Home() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center py-20">
      <div className="text-center space-y-3">
        ...
        <p className="text-sm text-gray-500 font-ui">Loading Veritas Nexus...</p>
      </div>
    </div>
  );
  ...
  const agricultureArticles = articles.filter((a: any) => a.primarySection === 'Agriculture').slice(0, 3);
  const trackerItems = policies.slice(0, 4);
  const timelineItems = timelines.slice(0, 3);
    ...
    <div className="space-y-10">
      <section>
        <div className="flex items-center justify-between mb-6">
          ...
          <Link to="/national" className="text-sm text-green hover:text-green-dark flex items-center gap-1 font-ui">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-5">
            {latestArticles.map((a: any) => <ArticleCard key={a.id} article={a} />)}
          </div>
        </div>
      </section>

      {agricultureArticles.length > 0 && (
        <section className="border-t border-gray-200 pt-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/agriculture" className="text-sm text-green hover:text-green-dark flex items-center gap-1 font-ui">
              View all <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {agricultureArticles.map((a: any) => <ArticleCard key={a.id} article={a} />)}
          </div>
        </section>
      )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  {['Project', 'Budget', 'Status', 'Agency'].map(h => (
                    <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-gray-600 uppercase font-ui">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {trackerItems.map((p: any) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <Link to={`/policy-tracker/${p.slug}`} className="font-medium text-gray-900 hover:text-green transition-colors">
                        {p.name}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600 font-ui">₱{p.budget}</td>
                    <td className="px-4 py-3">
                      <span className={`status-badge status-${p.status?.toLowerCase()}`}>{p.status}</span>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600 font-ui">{p.agency}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {timelineItems.length > 0 && (
        <section className="border-t border-gray-200 pt-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/timelines" className="text-sm text-green hover:text-green-dark flex items-center gap-1 font-ui">
              View all <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
            ...
            {timelineItems.map((t: any) => {
              ...
              return (
                <Link key={t.id} to={`/timelines/${t.slug}`}
                  ...
                  <h3 className="font-display font-semibold text-green-dark">{t.name}</h3>
      ...
      <section className="border-t border-gray-200 pt-8 pb-8">
        <div className="bg-green-light rounded-lg p-6 lg:p-8">
          <h2 className="text-xl font-display font-bold text-green-dark mb-3">News + Context + Data</h2>
          <p className="text-gray-700 max-w-2xl">
            Veritas Nexus is a Philippine news intelligence platform built for readers who want to
            understand, not just scroll.
          </p>
          <div className="flex flex-wrap gap-4 mt-5">
            <Link to="/topics" className="inline-flex items-center gap-2 px-4 py-2 bg-green text-white rounded-lg hover:bg-green-dark transition-colors font-ui text-sm">
              <Target className="w-4 h-4" />Explore Topics
            </Link>
            <Link to="/documents" className="inline-flex items-center gap-2 px-4 py-2 bg-white text-green border border-green rounded-lg hover:bg-green-light transition-colors font-ui text-sm">
              <FileText className="w-4 h-4" />Document Archive
            </Link>
          </div>
        </div>
      </section>
